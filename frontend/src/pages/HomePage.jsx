...
const HomePage = () => {
  const { data: statsData } = useQuery(GET_TRANSACTIONS_STATISTICS);
  ...
  useEffect(() => {
    if (statsData?.categoryStatistics) {
      ...
      const totalAmount = statsData?.categoryStatistics?.map(
  ...
  const [logout, { error, loading, data, client }] = useMutation(LOGOUT, {
